...
#### Popup Block/Timeout Fix
- **แก้ไขปัญหา Popup ถูก Block หรือปิดเร็วเกินไป**
  - เพิ่ม Auto-fallback ไปใช้ `signInWithRedirect` เมื่อ Popup ถูก Block
  - เพิ่มปุ่ม "เข้าสู่ระบบแบบ Redirect" สำหรับผู้ใช้ที่ Popup ไม่ทำงาน
  - รองรับ `getRedirectResult` สำหรับจัดการ Login ผ่าน Redirect
  - แก้ไขใน `src/lib/firebase/auth.ts` - เพิ่ม redirect fallback
  - แก้ไขใน `src/context/AuthContext.tsx` - เพิ่ม redirect result handler
  - แก้ไขใน `src/app/login/page.tsx` - เพิ่มปุ่มและ UI ใหม่

#### UX Improvements
- **ปรับปรุง Error Messages** - แสดงข้อความภาษาไทยที่เข้าใจง่าย
- **เพิ่ม Loading State** - แสดงสถานะขณะกำลัง Login
- **เพิ่ม Error Alert** - แสดง error ชัดเจนพร้อมคำแนะนำ
...
#### Google OAuth Scope Removed
- **ปิดการขอสิทธิ์ Google Calendar** - ลบ `calendar.events` scope ออกเพื่อแก้ปัญหา "แอปนี้ยังไม่ได้รับการยืนยันโดย Google"
  - แก้ไขใน `src/lib/firebase/auth.ts`
  - แก้ไขใน `src/lib/google/calendar-helper.ts`
  - ผู้ใช้สามารถ Login ได้โดยไม่ต้องเห็นหน้าจอ "Unverified App"
  - **หมายเหตุ**: ฟีเจอร์ Sync ปฏิทินถูกปิดใช้งานชั่วคราว

---

## [3.0.5] - 2026-01-21

### 👥 User Management & Dark Mode Improvements

#### User Management Enhancements
- **Online Status Indicator** - แสดงสัญลักษณ์ออนไลน์/ออฟไลน์ที่หน้าชื่อ User ในตาราง
  - สัญลักษณ์เขียวพร้อม animation สำหรับผู้ใช้ออนไลน์
  - สัญลักษณ์เทาสำหรับผู้ใช้ออฟไลน์
  - Tooltip แสดงสถานะเมื่อ hover
- **Online Status Filter** - Filter กรองผู้ใช้ตามสถานะออนไลน์
  - เลือกดูเฉพาะผู้ใช้ออนไลน์
  - เลือกดูเฉพาะผู้ใช้ออฟไลน์
  - ดูทั้งหมด
- **Sorting by Name** - เรียงลำดับตามชื่ออัตโนมัติ (A-Z)
- **Online History in User Details** - แสดงประวัติการ Online ในหน้าแก้ไขผู้ใช้
  - แสดงเวลาเข้าใช้งานล่าสุด
  - แสดงระยะเวลาที่ออนไลน์มาแล้ว (สำหรับผู้ใช้ออนไลน์)
  - แสดงเวลาที่เห็นล่าสุด

#### Dark Mode Improvements
- **Enhanced Readability** - ปรับปรุง Dark Mode ให้อ่านง่ายขึ้น
  - ปรับสีตัวหนังสือ (`foreground`) จาก `oklch(0.95 0 0)` เป็น `oklch(0.98 0 0)` เพื่อความคมชัด
  - ปรับสีพื้นหลัง (`background`) จาก `oklch(0.15 0 0)` เป็น `oklch(0.12 0 0)` เพื่อความสบายตา
  - ปรับสี Card และ Popover ให้เหมาะสม
  - ปรับสี `muted-foreground` จาก `oklch(0.75 0 0)` เป็น `oklch(0.85 0 0)` เพื่อความชัดเจน
  - ปรับสี Border และ Input ให้ชัดเจนขึ้น
  - ปรับ Contrast Ratio สำหรับการอ่านที่ดีขึ้น

---
## [3.0.4] - 2026-01-21

### 🧭 Stability & Meta Sync
- อัปเดตหมายเลขเวอร์ชันและ metadata ทุกแหล่งให้ตรงกับ deployment ล่าสุด (3.0.4)

---

## [3.0.3] - 2026-01-21

### 🔧 Performance & Presence Fix

- ปรับปรุงระบบ Presence ให้ทำงานทั่วทั้งแอปผ่าน `AuthContext` (ไม่จำกัดแค่ Dashboard)
- ปุ่ม Online Users ใช้ข้อมูลจาก Firestore presence (ผ่าน `presence-store`) ตัวเลขผู้ใช้ออนไลน์จึงตรงกับความเป็นจริง
- แก้ UI ให้กรองผู้ใช้ที่ไม่มี activity เกิน 5 นาทีออกจากรายการออนไลน์
- Sync ข้อมูลเวอร์ชันทั้งหมดเป็น `3.0.3` (`package.json`, `package-lock.json`, README, TODO)
